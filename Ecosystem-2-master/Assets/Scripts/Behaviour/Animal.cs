    ...
    void HandleInteractions () {
        if (currentAction == CreatureAction.Eating) {
            if (foodTarget && hunger > 0) {
                ...
                //Debug.Log("This is the food for  " + this.diet
                if(this.diet == Species.Rabbit)
				{
                    foodTarget.Die(CauseOfDeath.Eaten);
                    //eatAmount = 0.01f;
                    eatAmount = Mathf.Max(0, Mathf.Min(1, eatAmount));
                    //eatAmount = 0f;
                    //                   Die(CauseOfDeath.Eaten);
                    //foodTarget.d
                    //Debug.Log(foodTarget.)
                }
					//eatAmount = ((Rabbit)foodTarget).Consume(eatAmount);
                else
